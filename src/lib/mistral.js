...
// Pas besoin d'instancier le client si on utilise fetch via le proxy
...
export async function generatePharmacistAnalysis(answers, questions, userInfo) {
    try {
        ...
        // 2. Construire le prompt (avec 1000 mots max)
...
3. **Format et Longueur :**
   - La réponse totale doit faire **MOINS DE 1000 MOTS**.
...
### TA RÉPONSE
Analyse les réponses fournies ci-dessus et génère les conseils pharmaciens correspondants (max 1000 mots, sources gouvernementales uniquement).`;

        // 3. Appeler Mistral via le Proxy Vite
        // '/mistral-api' est redirigé vers 'https://api.mistral.ai/v1' par vite.config.js
        console.log("Envoi de la requête à Mistral via Proxy...");
        const response = await fetch('/mistral-api/chat/completions', {
            method: 'POST',
            headers: {
